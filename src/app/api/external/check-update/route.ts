...
export async function GET(request: Request) {
  try {
    ...
    if (!appIdStr && !packageName) {
      return NextResponse.json({ error: 'Missing appId or packageName' }, { status: 400 });
    }
    ...
    if (appIdStr) {
      ...
      let appQuery = db.select().from(apps).where(eq(apps.package_name, packageName));
      if (platform) {
        appQuery = db.select().from(apps).where(
          and(
    ...
    if (!latest) {
      return NextResponse.json({ error: 'No versions available for this app' }, { status: 404 });
    }
    ...
    let installUrl = '';
    let directDownloadUrl = `${baseUrl}/api/download?versionId=${latest.id}`;
